...
## 10) Troubleshooting: button appears but modal does not open

This is the most common issue.

### Why it happens
A modal **must** be opened as a direct response to the button interaction.
If you call `deferUpdate()` (or delay too long) before `showModal(...)`, Discord rejects it.

### Correct pattern
```js
if (!i.isButton() || i.customId !== 'open_form') return;
await i.showModal(modal); // must be immediate/direct
```

### Avoid this pattern
```js
await i.deferUpdate();
await i.showModal(modal); // ❌ too late, modal may not open
```

### Safe submit flow for beginners
1. Button click -> `showModal(modal)` immediately.
2. `await i.awaitModalSubmit(...)` with user+customId filter.
3. `submitted.deferReply({ ephemeral: true })`.
4. Send result to target channel.
5. `submitted.editReply('✅ Form submitted...')`.
